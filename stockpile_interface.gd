...
onready var create_stockpile_button = preload("res://CreateStockpile.tscn")
...
func _on_CreateStockpile_pressed() -> void:
	var new_stockpile_generator = load("res://StockpileType.tscn").instance()
...
#	print(Global.map_data[chunk_cell][grid_coord])
	if Global.map_data[chunk_cell][grid_coord].has("stockpile") and edit_mode:
		stockpile_selected = Global.map_data[chunk_cell][grid_coord].stockpile
