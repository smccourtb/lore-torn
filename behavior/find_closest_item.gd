...
func _tick(agent: CharacterController, _blackboard: Blackboard) -> bool:
	...
	while not items_on_ground.empty():
		...
		var item_to_haul: int = items_on_ground[closest_item_pos]
		...
		var item: Item = instance_from_id(item_to_haul)
		...
		if !stockpile.empty():
			...
			_blackboard.set_data("target_item", selected_item)
			return succeed()
	return fail()
